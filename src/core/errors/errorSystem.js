/**
 * Consistent Error Handling System for ZyraCSS
 * Establishes unified error contracts across all modules
 */
...
export class ZyraResult {
  ...
  /**
   * Chain operations that return ZyraResults
   */
  then(fn) {
    if (!this.success) {
      return this;
...
/**
 * Standard error codes used throughout ZyraCSS
 */
...
/**
 * Enhanced validation result with consistent error handling
 */
export class ValidationResult {
  ...
  static fromZyraResult(result) {
    if (result.success) {
      return ValidationResult.valid(result.data);
    } else {
...
/**
 * Enhanced parsing result with consistent error handling
 */
...
/**
 * Standardized CSS generation result
 */
...
if (typeof process !== "undefined") {
  ...
  process.on("uncaughtException", (error) => {
    ...
    // Don't exit the process for library code
  });
}
