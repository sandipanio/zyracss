...
export function processInput(input, options = {}) {
  ...
  if (sanitize && validStrings.length > 0) {
    ...
    // Add sanitization failures to invalid list
    sanitizeResult.failed.forEach((failed) => {
      result.invalid.push({
        input: failed,
...
/**
 * Specialized processor for class arrays
 * @param {Array} classes - Array of CSS class names
 * @param {Object} options - Processing options
 * @returns {Object} Class processing result
 */
export function processClassArray(classes, options = {}) {
  const result = processInput(classes, {
    expectArray: true,
    sanitize: true,
    deduplicate: true,
    includeStats: true,
  ...
  if (result.success && result.processed.length > 0) {
    result.analysis = analyzeClassPatterns(result.processed);
  }
...
/**
 * Analyze CSS class patterns for insights
 * @param {Array} classes - Array of processed class names
...
function analyzeClassPatterns(classes) {
  ...
  return {
    patterns,
    issues,
    dominantPattern: Object.entries(patterns).reduce((a, b) =>
...
export async function processHTMLInput(html, options = {}) {
  ...
  if (extractClasses && baseResult.success) {
    try {
      ...
      const { extractClassesFromHTMLArray } = await import(
        "../core/parser/htmlExtractor.js"
      );
      const extractedClasses = extractClassesFromHTMLArray(
