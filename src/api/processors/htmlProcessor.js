...
export function processHTMLInput(htmlArray, options = {}) {
  ...
  if (!Array.isArray(htmlArray)) {
    return ZyraResult.error(
      ErrorFactory.invalidInput(htmlArray, "HTML input must be an array", [
        "Provide an array of HTML strings",
        "Example: ['<div class=\"m-4\">Content</div>']",
      ])
    );
  }
  ...
  try {
    ...
    return ZyraResult.error(
      ErrorFactory.parsingFailed("HTML processing failed", error.message, [
        "Check HTML syntax",
...
export function validateHTMLContent(html) {
  if (typeof html !== "string") {
    return ZyraResult.error(
      ErrorFactory.invalidInput(html, "HTML content must be a string", [
