...
import {
  generateCSSFromValidatedClasses,
  compileFinalResult,
 ...
 */
export async function generateCSS(input, options = {}) {
  const timer = createTimer("generateCSS").start();
  ...
  try {
    ...
    const cssResult = await generateCSSFromValidatedClasses(
    ...
    return ZyraResult.error(
      ErrorFactory.invalidInput(
        ...
        [
          ...
          "Example: ['m-4', 'p-2', 'text-center']",
        ]
      )
    );
  }

  return generateCSS(classes, options);
}
...
export async function generateCSSFromHTML(html, options = {}) {
  ...
  return generateCSS(html, options);
}

/**
 * Convenience method for quick CSS generation (sync)
 * @param {string|string[]} input - Classes or HTML
 * @param {Object} options - Configuration options
 * @returns {string} Generated CSS string (empty string on error)
 */
export function quickCSS(input, options = {}) {
  try {
    const result = generateCSS(input, options);
    return result.success ? result.data.css : "";
  } catch (error) {
    return "";
  }
}
